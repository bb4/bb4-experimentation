...
/**
  ...
  */
object PrisonersAndSwitchSimulator:

  def main(args: Array[String]): Unit =
    // specified a seed parameter to Random if you want repeatable results
    val rnd = new Random()

    val numPrisoners = Input.getLong("How many prisoners?[1 - 500]:", 1, 500).toInt
    val numTrials = Input.getLong("How many trials should be run?[1 - 1000]:", 1, 1000).toInt
    val strategyIdx =
      Input
        .getLong(
    ...
    val stats = runTrials(numTrials, strategyIdx, numPrisoners, rnd)

    println("Summary statistics for " + numTrials + " trials of " + numPrisoners + " prisoners playing the game.")
    println(getStatsString(stats))

  def runTrials(numTrials: Int, strategyIdx: Int, numPrisoners: Int, rnd: Random): IntSummaryStatistics =
    val stats = new IntSummaryStatistics()

    for i <- 0 until numTrials do
      val strategy: PrisonerStrategy = strategyIdx match
