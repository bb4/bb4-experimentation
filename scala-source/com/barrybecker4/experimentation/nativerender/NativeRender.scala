...
object NativeRender:
  ...
  def main(args: Array[String]): Unit =
    ...
    if graphicsDevice.isFullScreenSupported then
  ...
  private def showDisplayModes(displayModes: Array[DisplayMode]): Unit =
    println("There are " + displayModes.length + " different display modes on this device.")
    var i = 0
    while i < displayModes.length do
  ...
  private def drawShapesInEachDisplayMode(
      graphicsDevice: GraphicsDevice,
      displayModes: Array[DisplayMode],
      frame: JFrame
  ): Unit =
    var lastWidth = -1
    for displayMode <- displayModes do
      if displayMode.getWidth != lastWidth && displayMode.getWidth > 1200 then
        lastWidth = displayMode.getWidth
        if graphicsDevice.isDisplayChangeSupported then
  ...
  private def drawShapes(frame: JFrame): Unit =
    ...
    while i < 2000 do
      ...
      g.fillRoundRect(
        ...
        10 + RANDOM.nextInt(RECT_WIDTH_VARIANCE),
        10 + RANDOM.nextInt(RECT_HEIGHT_VARIANCE),
        20,
        20
      )
      i += 1
