...
object PrimeNumberUtil:
  ...
  def isPrime(num: Long): Boolean =
    ...
    var i = 3L
    val limit = Math.sqrt(num.toDouble)
    while i <= limit do
  ...
  def isPrimeUnder1B(num: Long): Boolean =
    if num == 2 || num == 3 || num == 5 then return true
    if num % 2 == 0 || num % 3 == 0 || num % 5 == 0 then return false
    ...
    while true do
      ...
      while index < 24 do
        ...
        if v1 > sqrt then return true
        if num % v1 == 0 then return false
        index += 1
      c += 1
